...
public class BaseWorkerComponent_Prototipo : CharacterComponent
{

    protected RandomWalkLocomotion_Prototipo locomotion;
    public BaseWorkerModel_Prototipo model;
    public bool isWorking;
    protected override void Awake() {
        base.Awake();
        locomotion = GetComponent<RandomWalkLocomotion_Prototipo>();
        model = GetComponent<BaseWorkerModel_Prototipo>();
    }
    ...
    private void Work()
    {
        if (Vector3.Distance(this.transform.position, this.model.AsignatedStation.workerPosition.transform.position) < 3) {
            if (this.model.AsignatedStation.clientUsing == 2) {
                ...
                //    }
                //}
                model.AsignatedStation.accessToWork(this.name);
                this.isWorking = true;
